...
def test_bulk_import_shifts_alignment():
    ...
    assert np.array_equal(data["x"][:, 0], np.arange(6))
    # row t now holds what led to obs t; row 0 is the dummy reset row and
    # the source's final (unusable) action/reward are dropped
    assert np.array_equal(data["action"][:, 0], [0, 0, 10, 20, 30, 40])
    assert np.array_equal(data["reward"], [0, 0, 100, 200, 300, 400])
...
def test_action_out_writer_with_final_observation():
    ...
    assert len(episode) == 5 and episode.terminated
    data = episode.read()
    assert np.array_equal(data["x"][:, 0], [0, 1, 2, 3, 4])
    assert np.array_equal(data["action"][:, 0], [0, 0, 10, 20, 30])
    assert np.array_equal(data["reward"], [0, 0, 100, 200, 300])
...
def test_action_out_writer_drops_pending_without_final_observation():
    ...
    data = episode.read()
    assert np.array_equal(data["action"][:, 0], [0, 0, 10, 20])
